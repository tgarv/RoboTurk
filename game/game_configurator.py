    ...
    def get_player(self, color):
        ...
        while not player_type:
            try:
                player_type = int(
                    input(
                        "Select player type for %s: 1) Human (CLI) 2) Computer 3) Human (Board) \n"
                        % (color)
                    )
                )
        ...
        if player_type == player.Player.COMPUTER:
            difficulty = float(
                input("Select difficulty for %s player (1-10) \n" % (color))
            )
            return computer_player.ComputerPlayer(color, player_type, difficulty)
